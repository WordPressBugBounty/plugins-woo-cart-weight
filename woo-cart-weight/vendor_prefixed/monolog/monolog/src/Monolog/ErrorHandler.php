...
class ErrorHandler
{
    ...
    /** @var LogLevel::* */
    private $fatalLevel = \WCWeightVendor\Psr\Log\LogLevel::ALERT;
    /** @var ?string */
    ...
    public function __construct(\WCWeightVendor\Psr\Log\LoggerInterface $logger)
    {
        $this->logger = $logger;
    }
    /**
     ...
     * @return ErrorHandler
     */
    public static function register(\WCWeightVendor\Psr\Log\LoggerInterface $logger, $errorLevelMap = [], $exceptionLevelMap = [], $fatalLevel = null) : self
    {
    ...
    public function registerFatalHandler($level = null, int $reservedMemorySize = 20) : self
    {
        ...
        $this->fatalLevel = null === $level ? \WCWeightVendor\Psr\Log\LogLevel::ALERT : $level;
    ...
    {
        return ['ParseError' => \WCWeightVendor\Psr\Log\LogLevel::CRITICAL, 'Throwable' => \WCWeightVendor\Psr\Log\LogLevel::ERROR];
    }
    /**
    ...
    {
        return [\E_ERROR => \WCWeightVendor\Psr\Log\LogLevel::CRITICAL, \E_WARNING => \WCWeightVendor\Psr\Log\LogLevel::WARNING, \E_PARSE => \WCWeightVendor\Psr\Log\LogLevel::ALERT, \E_NOTICE => \WCWeightVendor\Psr\Log\LogLevel::NOTICE, \E_CORE_ERROR => \WCWeightVendor\Psr\Log\LogLevel::CRITICAL, \E_CORE_WARNING => \WCWeightVendor\Psr\Log\LogLevel::WARNING, \E_COMPILE_ERROR => \WCWeightVendor\Psr\Log\LogLevel::ALERT, \E_COMPILE_WARNING => \WCWeightVendor\Psr\Log\LogLevel::WARNING, \E_USER_ERROR => \WCWeightVendor\Psr\Log\LogLevel::ERROR, \E_USER_WARNING => \WCWeightVendor\Psr\Log\LogLevel::WARNING, \E_USER_NOTICE => \WCWeightVendor\Psr\Log\LogLevel::NOTICE, \E_STRICT => \WCWeightVendor\Psr\Log\LogLevel::NOTICE, \E_RECOVERABLE_ERROR => \WCWeightVendor\Psr\Log\LogLevel::ERROR, \E_DEPRECATED => \WCWeightVendor\Psr\Log\LogLevel::NOTICE, \E_USER_DEPRECATED => \WCWeightVendor\Psr\Log\LogLevel::NOTICE];
    }
    /**
    ...
    {
        $level = \WCWeightVendor\Psr\Log\LogLevel::ERROR;
        ...
        $this->logger->log($level, \sprintf('Uncaught Exception %s: "%s" at %s line %s', \WCWeightVendor\Monolog\Utils::getClass($e), $e->getMessage(), $e->getFile(), $e->getLine()), ['exception' => $e]);
    ...
    public function handleError(int $code, string $message, string $file = '', int $line = 0, ?array $context = []) : bool
    {
        ...
        if (!$this->hasFatalErrorHandler || !\in_array($code, self::$fatalErrors, \true)) {
            $level = $this->errorLevelMap[$code] ?? \WCWeightVendor\Psr\Log\LogLevel::CRITICAL;
