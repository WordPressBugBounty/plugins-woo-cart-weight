 ...
 */
class FingersCrossedHandler extends \WCWeightVendor\Monolog\Handler\Handler implements \WCWeightVendor\Monolog\Handler\ProcessableHandlerInterface, \WCWeightVendor\Monolog\ResettableInterface, \WCWeightVendor\Monolog\Handler\FormattableHandlerInterface
{
    ...
    public function __construct($handler, $activationStrategy = null, int $bufferSize = 0, bool $bubble = \true, bool $stopBuffering = \true, $passthruLevel = null)
    {
        if (null === $activationStrategy) {
            $activationStrategy = new \WCWeightVendor\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy(\WCWeightVendor\Monolog\Logger::WARNING);
        }
        // convert simple int activationStrategy to an object
        if (!$activationStrategy instanceof \WCWeightVendor\Monolog\Handler\FingersCrossed\ActivationStrategyInterface) {
            $activationStrategy = new \WCWeightVendor\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy($activationStrategy);
        }
        ...
        if ($passthruLevel !== null) {
            $this->passthruLevel = \WCWeightVendor\Monolog\Logger::toMonologLevel($passthruLevel);
        }
