 ...
 */
class LogglyHandler extends \WCWeightVendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     */
    public function __construct(string $token, $level = \WCWeightVendor\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
        if (!\extension_loaded('curl')) {
            throw new \WCWeightVendor\Monolog\Handler\MissingExtensionException('The curl extension is needed to use the LogglyHandler');
        }
        $this->token = $token;
    ...
    protected function send(string $data, string $endpoint) : void
    {
        ...
        \WCWeightVendor\Monolog\Handler\Curl\Util::execute($ch, 5, \false);
    }
    protected function getDefaultFormatter() : \WCWeightVendor\Monolog\Formatter\FormatterInterface
    {
        return new \WCWeightVendor\Monolog\Formatter\LogglyFormatter();
    }
}
