 ...
 */
class ChromePHPHandler extends \WCWeightVendor\Monolog\Handler\AbstractProcessingHandler
{
    ...
    {
        return new \WCWeightVendor\Monolog\Formatter\ChromePHPFormatter();
    }
    /**
    ...
    protected function send() : void
    {
        ...
        $json = \WCWeightVendor\Monolog\Utils::jsonEncode(self::$json, \WCWeightVendor\Monolog\Utils::DEFAULT_JSON_FLAGS & ~\JSON_UNESCAPED_UNICODE, \true);
        $data = \base64_encode($json);
        if (\strlen($data) > 3 * 1024) {
            self::$overflowed = \true;
            $record = ['message' => 'Incomplete logs, chrome header size limit reached', 'context' => [], 'level' => \WCWeightVendor\Monolog\Logger::WARNING, 'level_name' => \WCWeightVendor\Monolog\Logger::getLevelName(\WCWeightVendor\Monolog\Logger::WARNING), 'channel' => 'monolog', 'datetime' => new \DateTimeImmutable(), 'extra' => []];
