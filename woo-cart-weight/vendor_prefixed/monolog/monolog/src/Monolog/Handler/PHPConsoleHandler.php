 ...
 */
class PHPConsoleHandler extends \WCWeightVendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     */
    public function __construct(array $options = [], ?\WCWeightVendor\PhpConsole\Connector $connector = null, $level = \WCWeightVendor\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
    ...
    private function initConnector(?\WCWeightVendor\PhpConsole\Connector $connector = null) : \WCWeightVendor\PhpConsole\Connector
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \WCWeightVendor\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \WCWeightVendor\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\WCWeightVendor\PhpConsole\Helper::isRegistered()) {
            \WCWeightVendor\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \WCWeightVendor\PhpConsole\Handler::getInstance();
    ...
    public function getConnector() : \WCWeightVendor\PhpConsole\Connector
    {
    ...
    {
        if ($record['level'] < \WCWeightVendor\Monolog\Logger::NOTICE) {
    ...
    private function handleDebugRecord(array $record) : void
    {
        ...
        if ($record['context']) {
            $message .= ' ' . \WCWeightVendor\Monolog\Utils::jsonEncode($this->connector->getDumper()->dump(\array_filter($record['context'])), null, \true);
        }
    ...
    {
        return new \WCWeightVendor\Monolog\Formatter\LineFormatter('%message%');
    }
}
