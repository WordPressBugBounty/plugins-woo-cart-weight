 ...
 */
class BrowserConsoleHandler extends \WCWeightVendor\Monolog\Handler\AbstractProcessingHandler
{
    /** @var bool */
    ...
    {
        return new \WCWeightVendor\Monolog\Formatter\LineFormatter('[[%channel%]]{macro: autolabel} [[%level_name%]]{font-weight: bold} %message%');
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        return [\WCWeightVendor\Monolog\Logger::DEBUG => 'debug', \WCWeightVendor\Monolog\Logger::INFO => 'info', \WCWeightVendor\Monolog\Logger::NOTICE => 'info', \WCWeightVendor\Monolog\Logger::WARNING => 'warn', \WCWeightVendor\Monolog\Logger::ERROR => 'error', \WCWeightVendor\Monolog\Logger::CRITICAL => 'error', \WCWeightVendor\Monolog\Logger::ALERT => 'error', \WCWeightVendor\Monolog\Logger::EMERGENCY => 'error'][$level] ?? 'log';
    }
    /**
     * @return string[]
     */
    private static function handleStyles(string $formatted) : array
    {
        ...
        foreach (\array_reverse($matches) as $match) {
            ...
            $pos = $match[0][1];
            $format = \WCWeightVendor\Monolog\Utils::substr($format, 0, $pos) . '%c' . $match[1][0] . '%c' . \WCWeightVendor\Monolog\Utils::substr($format, $pos + \strlen($match[0][0]));
        }
    ...
    private static function handleCustomStyles(string $style, string $string) : string
    {
        ...
        if (null === $style) {
            ...
            throw new \RuntimeException('Failed to run preg_replace_callback: ' . $pcreErrorCode . ' / ' . \WCWeightVendor\Monolog\Utils::pcreLastErrorMessage($pcreErrorCode));
        }
        return $style;
    }
    /**
