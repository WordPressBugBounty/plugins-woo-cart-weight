 ...
 * .
 */
class Page implements \WCWeightVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
     ...
     */
    public function __construct(string $assets_url, \WCWeightVendor\Octolize\ShippingExtensions\Tracker\ViewPageTracker $view_page_tracker)
    {
    ...
    {
        \wp_enqueue_style(\WCWeightVendor\Octolize\ShippingExtensions\Assets::HANDLE);
    ...
    {
        $plugins = \WCWeightVendor\Octolize\ShippingExtensions\Plugin\PluginFactory::get_plugins();
        list($plugins_priority, $categories_priority) = $this->get_plugins_and_categorites_priority();
        $sorter = new \WCWeightVendor\Octolize\ShippingExtensions\Plugin\PluginSorter($plugins, $plugins_priority, $categories_priority);
    ...
    private function get_plugins_and_categorites_priority() : array
    {
        ...
        $categories = \WCWeightVendor\Octolize\ShippingExtensions\Plugin\PluginFactory::get_categories();
        $plugins_priority = [];
        $categories_priority = [];
        if (\in_array('flexible-shipping/flexible-shipping.php', $active_plugins, \true)) {
            ...
            $categories_priority[] = $categories[\WCWeightVendor\Octolize\ShippingExtensions\Plugin\PluginFactory::CATEGORY_CUSTOMIZABLE_RATES];
            $categories_priority[] = $categories[\WCWeightVendor\Octolize\ShippingExtensions\Plugin\PluginFactory::CATEGORY_SHIPPING_LABELS];
        }
        if (\in_array('flexible-shipping-ups/flexible-shipping-ups.php', $active_plugins, \true)) {
            ...
            $categories_priority[] = $categories[\WCWeightVendor\Octolize\ShippingExtensions\Plugin\PluginFactory::CATEGORY_LIVE_RATES];
        }
    ...
    {
        return \array_values(\WCWeightVendor\Octolize\ShippingExtensions\Plugin\PluginFactory::get_categories());
    }
}
