...
class OptInPage implements \WCWeightVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    /**
     * @var string
     */
    ...
    public function wpdesk_tracker_page()
    {
        ...
        $shop = new \WCWeightVendor\WPDesk\Tracker\Shop($shop_url);
        $terms_url = $shop->get_usage_tracking_page();
        $logo = $shop->get_shop_logo_file();
        $logo_url = \plugin_dir_url(__FILE__) . '../../../assets/images/' . $logo;
        $renderer = new \WCWeightVendor\WPDesk\View\Renderer\SimplePhpRenderer(new \WCWeightVendor\WPDesk\View\Resolver\DirResolver(__DIR__ . '/views'));
        // WPCS: XSS ok.
